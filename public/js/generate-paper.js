  ...
  if (role === "teacher") {
    ...
    if (role === "teacher" && teacher) {
        ...
        document.getElementById("className").innerHTML = `
            <option value="${teacher.classes[0]}">
                ${teacher.classes[0]}
            </option>
        `;
...
async function loadChapters() {
    ...
    const res = await fetch(
        `/api/questions/chapters?subject=${encodeURIComponent(subject)}&schoolId=${schoolId}&className=${className}`
    );

    const data = await res.json();

    console.log("Chapter API:", data);
...
document
.getElementById("generateOnlineTestBtn")
.addEventListener("click", generateOnlineTest);

async function generateOnlineTest() {

    if (!currentPaperId) {

        showToast("Please generate the paper first.", true);
    ...
    try {
        ...
        showToast("Creating Online Test...");
        ...
        showToast("Online Test Created Successfully");
...
Students of this class can now see this test after login.`
        ...
        showToast("Unable to create Online Test", true);
